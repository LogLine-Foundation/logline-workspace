...
impl CapsuleHeader {
    pub fn empty(dim: u16, flags: CapsuleFlags, cid: [u8; 32], len: u32, ts_ms: u64) -> Self {
        Self {
            magic: CAP_MAGIC,
            ver: CAP_VER,
            flags: flags.bits(),
    ...
    pub fn to_bytes_wo_sig(&self) -> [u8; HEADER_LEN - 64] {
        ...
        let mut i = 0usize;
        out[i..i + 2].copy_from_slice(&self.magic.to_le_bytes());
        i += 2;
        out[i..i + 1].copy_from_slice(&[self.ver]);
        i += 1;
        out[i..i + 1].copy_from_slice(&[self.flags]);
        i += 1;
        out[i..i + 8].copy_from_slice(&self.ts_ms.to_le_bytes());
        i += 8;
        out[i..i + 32].copy_from_slice(&self.cid);
        i += 32;
        out[i..i + 2].copy_from_slice(&self.dim.to_le_bytes());
        i += 2;
        out[i..i + 4].copy_from_slice(&self.len.to_le_bytes());
        i += 4;
    ...
    pub fn from_bytes(raw: &[u8]) -> Result<Self, LllvError> {
        ...
        let mut i = 0usize;
        let magic = u16::from_le_bytes(raw[i..i + 2].try_into().unwrap());
        i += 2;
        let ver = raw[i];
        i += 1;
        let flags = raw[i];
        i += 1;
        let ts_ms = u64::from_le_bytes(raw[i..i + 8].try_into().unwrap());
        i += 8;
        let mut cid = [0u8; 32];
        cid.copy_from_slice(&raw[i..i + 32]);
        i += 32;
        let dim = u16::from_le_bytes(raw[i..i + 2].try_into().unwrap());
        i += 2;
        let len = u32::from_le_bytes(raw[i..i + 4].try_into().unwrap());
        i += 4;
        let mut sig = [0u8; 64];
