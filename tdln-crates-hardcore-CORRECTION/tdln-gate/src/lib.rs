...
use thiserror::Error;
use tdln_compiler::{compile, CompileCtx, CompiledIntent};

#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
pub enum Decision { Allow, Deny, NeedsConsent }
...
pub fn preflight(intent: &CompiledIntent, _ctx: &PolicyCtx) -> Result<GateOutput, GateError> {
    ...
    let events = vec![
        LogEvent { kind: "policy.preflight".into(), payload: audit.clone() },
    ];
    Ok(GateOutput {
...
pub fn decide(intent: &CompiledIntent, consent: &Consent, ctx: &PolicyCtx) -> Result<GateOutput, GateError> {
