...
pub fn build_proof(ast: &SemanticUnit, canon_json: &[u8], rules: &[impl AsRef<str>]) -> ProofBundle {
...
pub fn verify_signatures(bundle: &ProofBundle, keys: &[VerifyingKey]) -> Result<(), ProofError> {
    ...
    let msg = bundle_digest(bundle);
    for (sig, vk) in bundle.signatures.iter().zip(keys.iter().cycle()) {
        vk.verify(&msg, sig).map_err(|_| ProofError::VerifyFailed)?;
    }
    Ok(())
}
