...
use crate::{
    ...
    hash::hex32,
    merkle::{leaf_hash_doc, prove_checked, root_checked, verify_path_checked, ProofStep},
    search::cosine,
};
use hex::FromHex;
...
impl IndexPackBuilder {
    ...
    /// Adiciona uma capsule ao pack.
    ///
    /// # Errors
    ///
    /// - `IndexError::DimMismatch` se a dimensão não coincidir
    ...
    /// Constrói o pack, opcionalmente selando o manifesto.
    ///
    /// # Errors
    ///
    /// - `IndexError::EmptyIndex` se nenhuma capsule foi adicionada
    /// - `IndexError::Capsule` se o payload não tiver o tamanho esperado
    /// - `IndexError::Merkle` se a árvore Merkle não puder ser construída
    /// - `IndexError::DimMismatch` se o payload não tiver a dimensão declarada
    /// - `IndexError::Serde` ao serializar o manifesto opcional
...
impl IndexPack {
    ...
    /// Consulta o índice retornando as melhores correspondências.
    ///
    /// # Errors
    ///
    /// - `IndexError::EmptyIndex` se o pack estiver vazio
    /// - `IndexError::DimMismatch` se o vetor de consulta tiver dimensão diferente
    /// - `IndexError::DimMismatch` se houver erro no cálculo do cosseno
    /// - `IndexError::Merkle` se a prova não puder ser construída
    pub fn query(&self, req: &QueryRequest, topk: usize) -> Result<QueryEvidence, IndexError> {
        ...
        let mut scores: Vec<(usize, f32)> = (0..self.vecs.len())
            .map(|i| Ok((i, cosine(&req.vec, &self.vecs[i])?)))
        ...
        for (idx, score) in scores.iter().take(k) {
            ...
            let (_root, path_raw) = prove_checked(&self.leaves[..], *idx)?;
    ...
    /// Verifica as provas e dimensões retornadas por uma consulta.
    ///
    /// # Errors
    ///
    /// - `IndexError::DimMismatch` se a consulta ou as evidências divergirem da dimensão
    /// - `IndexError::Merkle` se qualquer prova Merkle falhar
    pub fn verify(&self, ev: &QueryEvidence) -> Result<(), IndexError> {
        ...
        for r in &ev.results {
            ...
            verify_path_checked(leaf, &path_raw, self.root)
