...
fn build_levels(leaves: &[[u8; 32]]) -> Result<Vec<Vec<[u8; 32]>>, IndexError> {
    ...
    while let Some(cur) = levels.last() {
        if cur.len() <= 1 {
            break;
        }
...
/// # Errors
///
/// - `IndexError::Merkle` se `leaves` estiver vazio ou `idx` estiver fora do intervalo
...
/// # Errors
///
/// - `IndexError::Merkle` se o caminho reconstruído não bater com a raiz esperada
...
/// # Panics
///
/// Panics se `leaves` estiver vazio.
...
/// # Panics
///
/// Panics se `leaves` estiver vazio ou `idx` estiver fora do intervalo.
...
/// # Errors
///
/// - Propaga erros de [`verify_path_checked`]
pub fn verify_path(
    leaf: [u8; 32],
    path: &[ProofStep],
    expected_root: [u8; 32],
